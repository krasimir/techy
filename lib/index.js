...
var Techy = function(r, t, callback, config) {
	...
	var api = {
		...
		copyingThemeFolder: function(cb) {
			api.copyFolder(api.rootTechy + '/themes', api.root + '/themes', function() {
				cb();
			});
		},
		definingThemeFolder: function(cb) {
		...
		copyingPublicFolder: function(cb) {
		...
		compilePages: function(cb) {
			if(!this.pagesPattern) {
				this.pagesPattern = [api.config.src + '/**/*.md', '!' + api.config.src + '/node_modules/**'];
		...
		setMasterConfig: function(cb) {
			if(fs.existsSync(root + '/Techy.js')) {
				api.config = extend(config, require(root + '/Techy.js')());
			} if(fs.existsSync(api.themeDir + '/TechyFile.js')) {
				api.config = extend(config, require(api.themeDir + '/TechyFile.js')());
			} else if(fs.existsSync(root + '/TechyFile.js')) {
				api.config = extend(config, require(root + '/TechyFile.js')());
			} else if(config && config.techyFile && fs.existsSync(config.techyFile)) {
				api.config = extend(config, require(config.techyFile)());
			} else {
				api.config = config;
			}
			api.verifyThatSrcAndDestExists();
			api.publicDir = api.config.dest + '/public';
			cb();
		},
		verifyThatSrcAndDestExists: function(cb) {
			var check = function(path, throwException) {
			...
			if(!check(api.config.src)) {
				api.config.src = path.normalize(root + '/' + api.config.src);
				check(api.config.src, true);
			}
			if(!check(api.config.dest)) {
				api.config.dest = path.normalize(root + '/' + api.config.dest);
				check(api.config.dest, true);
			}
			cb && cb();		
		}
	}
	...
	queue([
		api.copyingThemeFolder,
		api.definingThemeFolder,
		api.setMasterConfig,
		api.creatingDestFolder,
		api.copyingPublicFolder,
		api.compileCSS,
		api.compileJS,
