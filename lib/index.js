	...
	var api = {
					...
					if(
						...
						if(stat.isDirectory()) {
							api.copyFolder(
								...
								true,
								function(file) {
									var normilized = path.normalize(file);
									var basenameFile = path.basename(normilized);
									var result = true;
									if(basenameFile.indexOf('_') === 0) {
										var stat = fs.statSync(normilized);
										if(stat.isDirectory(normilized)) {
											result = false;
										} else {
		...
		compileCSS: function(cb) {
			if(api.config.css) {
				...
				CSSDirToWatch = api.root + '/' + api.config.css.watch || index;
		...
		compileJS: function(cb) {
			if(api.config.js) {
				var pSrc = path.normalize(api.root + '/' + api.config.js.src);
				var pDest = path.normalize(api.root + '/' + api.config.js.dest);
				gulp.src(pSrc)
				// .pipe(uglify())
				.pipe(concat(path.basename(pDest)))
				// .pipe(jshint())
				// .pipe(jshint.reporter('default'))
				.pipe(gulp.dest(path.dirname(pDest)))
				.pipe(GulpEnd(cb));
		...
		watchFiles: function() {
			...
			if(api.config.js) {
				gulp.watch([path.normalize(api.root + '/' + api.config.js.src)], function(event) {
					this.compileJS();
				}.bind(this));
			}
...
module.exports.gulp = function(settings) {
	...
	return function() {
		return T.compileCSS(noop).compileJS(noop).factory.gulp();
	}
}
